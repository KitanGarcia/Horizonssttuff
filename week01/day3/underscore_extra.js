...
// value without calling 'fn'.
/* BONUS
function memoize(max, hashFunction)
{
  var results = {};
  return function()
  {
    var args = Array.from(arguments);//puts arguments in array form
    args = hashfunction(args);
    if (!results[args])
    {
      results[args] = max.apply(null, args);
    }
    return results[args];
  }
}

function hashFunction(input) {return input.join('--')}
*/
function memoize(func) {
  var cache = {};
  return function(arg1)//if arg1 is already a key in the cache
  {
    if (!cache.hasOwnProperty(arg1))//safer than cache[arg1] -> dangerous if == 0
    {
      cache[arg1] = func(arg1);//would need apply for multiple arguments
    }
      return cache[arg1];
  }
}
...
function partial(fn) {
  ...
  var setArgs = Array.from(arguments).slice(1);//Array.from to put arguments into an array!! slice return everything except first element (first is getargs so we don't care)
  return function()
  {
    var currentArgs = Array.from(arguments);//arguments of this new function
    var combinedArgs = setArgs.concat(currentArgs);//merge arrays
    return fn.apply(null, combinedArgs);//call fn on arguments in array form. null binding piece
  }
}
...
function composeBasic(fun1, fun2) {
  // YOUR CODE HERE
  return function composedfn(fun1, fun2)
  {
    return fun1(fun2());
  }
}
