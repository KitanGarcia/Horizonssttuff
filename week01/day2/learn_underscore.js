...
//
// Use _.some() from Underscore to build this.
// Underscore function _.some(array, fun) returns true if fun(item) returns true
// for ANY item in an array.
// ex.
//  function greaterThan1(item) {
//    return item > 1;
//  }
//  _.some([], greaterThan1) -> false
//  _.some([2], greaterThan1) -> true
//  _.some([0, 2], greaterThan1) -> true
//  _.some([0, -1, 0, -2], greaterThan1) -> false
//
...
// Exercise 1. learn_underscore.contains(array, item) using _.some()
...
learn_underscore.contains = function(array, item) {
  // YOUR CODE HERE
  return _.some(array, function(check) {
    return check === item;
  });
};

// Exercise 3: learn_underscore.some(array, fun)
// Write your own version of _.some() using _.reduce() and _.map();
//
...
//
// To build learn_underscore.some() you should first use _.map(array, fun) to convert
...
//     return !! item;
//   }
//   learn_underscore.some([], isTruthy) -> false
//   learn_underscore.some([0, 0], isTruthy) -> false
//   learn_underscore.some([0, 1, 0], isTruthy) -> true
//   learn_underscore.some([1], isTruthy) -> true
learn_underscore.some = function(array, fun) {
  // YOUR CODE HERE
  //_.some returns true if fun(item) = true;
  //_.map changes each element by passing it into fun
 // _.map(array, fun(item));
  function OR (a ,b)
  {
    return a || b;
  }
...
// Write a function using _.each() that takes an object and returns an array with
// all the keys in that object.
//
// ex. learn_underscore.keys({}) -> []
// ex. learn_underscore.keys({a: 1, hello: 10}) -> ['a', 'hello']
//
// Underscore function _.each() is similar to the Array.forEach() built-in function
...
learn_underscore.countBy = function(array, fun) {
  // YOUR CODE HERE
  return _.mapObject(_.groupBy(array, fun()), fun());//fix this
}
